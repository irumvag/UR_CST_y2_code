...
public:
    DDA(int x0, int y0, int x1, int y1) {
        ...
        for (int i = 0; i <= steps; ++i) {
            points.emplace_back(round(x), round(y));
            x += xInc;
            y += yInc;
            cout<< "Point: (" << round(x) << ", " << round(y) << ")" << endl; // Debug output
            cout<< "xInc: " << xInc << ", yInc: " << yInc << endl; // Debug output
        }
    }

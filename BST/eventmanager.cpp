...
public:
    ...
    AVLNode(DateTime dt, Event* event) 
        : key(dt), height(1), left(nullptr), right(nullptr) {
...
private:
    ...
    AVLNode* insertRec(AVLNode* node, Event* event) {
        if (!node) return new AVLNode(event->date_time, event);
    ...
    void rangeSearchRec(AVLNode* node, const DateTime& start, const DateTime& end,
...
public:
    ...
    vector<Event*> getUpcomingEvents(int days = 7) {
        ...
        return avl.rangeSearch(start, end, current_user ? current_user->id : -1);
    }
